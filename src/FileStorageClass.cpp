 ...
 *
 * The output corresponds to the standard 2016-06 OME-XML schema. To support fully arbitrary, 
 * out-of-order, and sparsely sampled acquisitions with potentially changing image dimensions 
 * across channels/time, each acquired 2D frame is written out sequentially as an entirely 
 * independent, isolated `<Image SizeC="1" SizeT="1">` block. 
 *
 ...
 *
 *  - Base Image Blocks: Each channel capture forms a standalone `<Image>`. Since OME-XML does not 
 *    natively support labeling disconnected sets as "Detection sequences" without rigidly grouping 
 *    them into dense 5D hyperstacks, we inject a JSON `<Description>` payload storing at least 
 *    the `DetectionIndex`. 
 *    The hardware and acquisition phase map tightly to `<Channel Name="AcquisitionName|DetectorName" />`.
 *    Spatial coordinates are bound to the inner `<Plane>` element, which also records the `DeltaT`.
 *    Each `<Image>` maps back to the global logic using `<AnnotationRef>`.
 *
...
std::string FileStorageClass::_generateOMEXML() const {
    ...
    // data structures that maintains sequence numbers for each channel
    std::map<AcqTypeAndDetName, int> nextIndices;
    for (const auto& id : _imageDimensions) {
        nextIndices[id.first] = 0;
    }

    // Loop over all acquired images in the order they were processed globally
    for (size_t i = 0; i < _imagesAcqTypesAndDetNames.size(); i += 1) {
        ...
        tinyxml2::XMLElement* descElem = xmlDoc.NewElement("Description");
        imageElem->InsertEndChild(descElem);
        nlohmann::json descJson = {
            {"DetectionIndex", detectionIndex},
            {"StagePositionName", stagePositionName}
        };
        descElem->SetText(descJson.dump().c_str());
